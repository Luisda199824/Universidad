...
def t_FLOAT(t):
    r'(((\d*\.\d+)|(\d+\.\d*))(e(\d+|\+\d+|-\d+))?)|(\d+e(\d+|\+\d+|-\d+))'        
    try:
        t.value = float(t.value)
    except ValueError:
        #print("El Valor Flotante no es correcto, linea : ", t.lexer.lineno)
        error(t.lexer.lineno,"El Valor Flotante no es correcto")
        t.value = 0
    return t
    ...
    if t.value[0] == "0":
        try:
            if (t.value[1] == 'x') or (t.value[1] == 'X'):
                ...
                #print "Pase por octal"
                t.value = int(t.value, 8)
            else:
                #print "Pase por decimal"
                t.value = int(t.value)
        except:
            #print("El Valor hexadecimal no es correcto, linea : ", t.lexer.lineno)
            error(t.lexer.lineno,"El Valor hexadecimal no es correcto")
            t.value = 0
    else:
        #print "Pase por decimal"
        t.value = int(t.value)
...
def _replace_escape_codes(t):
        ...
        if cont >= len(t.value):
            ...
            if t.value[cont] == '\\':
                try:
                    if t.value[cont+1] == 'b':
                        try:
                            string = t.value[cont+2:cont+4]
                            stringleft = t.value[0:cont]
                            stringright = t.value[cont+4::]
                            hexa = int(string,16)
                            t.value = stringleft + str(hexa) + stringright
                        except:
                            cont += 4
                            error(t.lexer.lineno,"El valor hexa \\bxx no es valido")
                    else:
                        if t.value[cont+1] != 'u':
                            error(t.lexer.lineno,"El codigo de escape " + str(t.value[cont])+ str(t.value[cont+1]) + " es invalido")
                except:
                    error(t.lexer.lineno,"El codigo de escape es invalido")
        cont += 1
    return t.value
    ...
    *** DEBE IMPLEMENTAR ***
    ...
    Reemplace todos los códigos de escape válidos \.. en una cadena con
    su código de caracter primitivo equivalenteself.
    '''
...
def t_STRING(t):
    ...
    #print t.value
    t.value = t.value[1:-1]
    t.value = _replace_escape_codes(t)    # Debe implementarse antes
    return t
...
def t_BOOLEAN(t):
    ...
    if t.value[0] == 't':
        ...
        error(t.lexer.lineno,"El Valor Booleano no es correcto")
    return t
...
def t_COMMENT_UNTERM(t):
    r'(\/\*(\s*|.*?)*)'
    error(t.lexer.lineno,"Comentario no cerrado")
    t.lexer.lineno += t.value.count('\n')

# Constantes de cadena no terminadas
def t_STRING_UNTERM(t):
    r'".*'
    error(t.lexer.lineno,"String incompleto")
    t.lexer.lineno += 1
